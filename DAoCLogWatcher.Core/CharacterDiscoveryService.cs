...
public static class CharacterDiscoveryService
{
	...
	{
		return GetCharacterNames(GetEdenPath());
	}
	...
	public static IReadOnlyList<string> GetCharacterNames(string edenPath)
	{
		...
		return Directory.EnumerateFiles(edenPath, "*.ini").Select(f => ServerSuffixRegex.Replace(Path.GetFileNameWithoutExtension(f), string.Empty)).Where(name => !string.IsNullOrWhiteSpace(name)).Distinct(StringComparer.OrdinalIgnoreCase).
