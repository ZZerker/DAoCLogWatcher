...
public sealed class WarmapRelicTests
{
	...
	[Fact]
	public void RelicsSnapshot_ParsesOwnerAndOrigin()
	{
	...
	[Fact]
	public void GetRelicPlacements_SkipsEmptyPads()
	{
		var service = MakeService();

		var placements = service.GetRelicPlacements();

		placements.Should().HaveCount(4);
		placements.Should().NotContain(p => p.PadName == "Castle Excalibur");
	}

	[Fact]
	public void GetRelicPlacements_CapturedRelic_IsNotAtHomeAndKeepsOriginRealm()
	{
		var service = MakeService();
	...
	[Fact]
	public void GetRelicPlacements_RelicOnOwnHomePad_IsAtHome()
	{
		var service = MakeService();
	...
	[Fact]
	public void GetRelicPlacements_UnknownRelicId_FallsBackToPadFields()
	{
		var service = new WarmapWebSocketService();
		service.ProcessMessage(PadsJson);
		...
		placement.PadName.Should().Be("Dun Crauchon");
		placement.OwnerRealm.Should().Be(3);
		placement.Type.Should().Be(0);
	}
}
