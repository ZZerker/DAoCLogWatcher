...
public sealed class CombatProcessorTests
{
	...
	public void MultiHit_SingleSpellHit_NeverFires()
	{
		...
		multiHits.Should().BeEmpty("only 2 unique targets hit");
	}
	...
	public void MultiHit_FiveUniqueTargets_FiresWhenNextSpellClosesWindow()
	{
		...
		multiHits.Should().HaveCount(1);
		multiHits[0].HitCount.Should().Be(5);
		multiHits[0].SpellName.Should().Be("Fireball");
	}
	...
	public void MultiHit_FourUniqueTargets_DoesNotFire()
	{
		...
		multiHits.Should().BeEmpty("4 targets is below the threshold of 5");
	}
	...
	public void MultiHit_WindowClosedByIncomingHeal_Fires()
	{
		...
		multiHits.Should().HaveCount(1, "window expired and should fire on any subsequent timestamped line");
	}
	...
	public void MultiHit_SameTargetHitMultipleTimes_CountedOnce()
	{
		...
		multiHits.Should().BeEmpty("5 hits on the same target counts as 1 unique target");
	}
	...
	public void MultiHit_WeaponHitBetweenAoeHits_DoesNotInterruptWindow()
	{
		...
		multiHits.Should().HaveCount(1, "weapon hit should not have split the Fireball window");
		multiHits[0].HitCount.Should().Be(5);
	}
	...
	public void MultiHit_DifferentSpellInterruptsWindowAndFinalizes()
	{
		...
		multiHits.Should().HaveCount(2, "each spell's AoE should produce its own multi-hit event");
	}
	...
	public void MultiHit_Reset_ClearsWindowWithoutFiring()
	{
		...
		multiHits.Should().BeEmpty("reset must clear the in-flight window");
	}
}
