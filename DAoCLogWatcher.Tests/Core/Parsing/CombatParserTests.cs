...
public sealed class CombatParserTests
{
	...
	public void TryParse_DealtHitWithAbsorbed_ReturnsPendingThenFlushed()
	{
		...
		var result = this.parser.TryParse("[21:06:19] You hit Vareth for 39 (-7) damage!", out var damage, out var heal, out _);
		...
		var result2 = this.parser.TryParse("[21:06:25] Some other line", out var damage2, out var heal2, out _);
		result2.Should().BeTrue();
		damage2.Should().NotBeNull();
		damage2!.Opponent.Should().Be("Vareth");
	...
	{
		this.parser.TryParse("[21:06:20] You hit Vareth for 39 (-7) damage!", out _, out _, out _);

		var result = this.parser.TryParse("[21:06:20] You critically hit for an additional 16 damage! (Crit Chance: 10%)", out var damage, out _, out _);
	...
	{
		this.parser.TryParse("[21:10:43] You hit the elder tangler for 532 damage!", out _, out _, out _);

		var result = this.parser.TryParse("[21:10:44] unrelated", out var damage, out _, out _);
		result.Should().BeTrue();
		damage!.BaseDamage.Should().Be(532);
		damage.Absorbed.Should().Be(0);
		damage.Opponent.Should().Be("the elder tangler");
	}

	[Fact]
	public void TryParse_ConsecutiveDealtHits_FlushesFirstOnSecond()
	{
		this.parser.TryParse("[21:06:40] You hit Drakthos for 250 (-193) damage!", out _, out _, out _);

		// Second hit flushes first (no crit for first)
		var result = this.parser.TryParse("[21:06:41] You hit Drakthos for 250 (-193) damage!", out var damage, out _, out _);
		result.Should().BeTrue();
		damage!.Opponent.Should().Be("Drakthos");
	...
	{
		var result = this.parser.TryParse("[20:56:16] Grimskar hits you for 121 (-42) damage!", out var damage, out _, out _);

		result.Should().BeFalse("hit is pending — crit may follow");
		damage.Should().BeNull();

		var result2 = this.parser.TryParse("[20:56:17] unrelated", out var damage2, out _, out _);
		result2.Should().BeTrue();
		damage2!.Opponent.Should().Be("Grimskar");
	...
	{
		this.parser.TryParse("[20:56:16] Grimskar hits you for 121 (-42) damage!", out _, out _, out _);

		// Taken crit has NO timestamp
		var result = this.parser.TryParse("Grimskar critically hits you for an additional 46 damage! (Crit Chance: 41%)", out var damage, out _, out _);

		result.Should().BeTrue();
		damage!.Opponent.Should().Be("Grimskar");
	...
	{
		var result = this.parser.TryParse("[20:56:44] You are healed by Thendria for 582 hit points.", out _, out var heal, out _);
	...
	public void TryParse_HealLine_FlushesStaleDealtPending()
	{
		...
		this.parser.TryParse("[21:00:00] You hit Enemy for 300 (-50) damage!", out _, out _, out _);

		var result = this.parser.TryParse("[21:00:01] You are healed by Ulfdan for 248 hit points.", out var damage, out var heal, out _);
	...
	{
		var result = this.parser.TryParse("[20:51:32] Quelris was just killed by Torven in Hadrian's Wall.", out var damage, out var heal, out _);
	...
	{
		var result = this.parser.TryParse("   ", out var damage, out var heal, out _);
	...
	{
		this.parser.TryParse("[21:06:19] You cast a Extinguish Lifeforce spell!", out _, out _, out _);
		this.parser.TryParse("[21:06:19] You hit Vareth for 39 (-7) damage!", out _, out _, out _);
	...
	{
		this.parser.TryParse("[21:06:21] You cast a Annihilate Strength spell!", out _, out _, out _);
		this.parser.TryParse("[21:06:21] You cast a Scatter Zeal spell!", out _, out _, out _);
		this.parser.TryParse("[21:06:21] You cast a Extinguish Lifeforce spell!", out _, out _, out _);
		this.parser.TryParse("[21:06:21] You hit Vareth for 39 (-7) damage!", out _, out _, out _);
	...
	{
		this.parser.TryParse("[21:06:19] You cast a Extinguish Lifeforce spell!", out _, out _, out _);
		this.parser.TryParse("[21:06:19] Vareth attacks you and your spell is interrupted!", out _, out _, out _);
		this.parser.TryParse("[21:06:20] You hit Vareth for 39 (-7) damage!", out _, out _, out _);
	...
	public void TryParse_AeSpell_AllHitsCarrySpellName()
	{
		...
		this.parser.TryParse("[21:16:01] You cast a Resonant Concussion spell!", out _, out _, out _);
		this.parser.TryParse("[21:16:01] You hit Draekon for 93 (-17) damage!", out _, out _, out _);

		// Flush first hit, start second
		this.parser.TryParse("[21:16:01] You hit Draekon for 93 (-17) damage!", out var first, out _, out _);
		first!.SpellName.Should().Be("Resonant Concussion", "first AE hit should carry spell name");

		// Flush second, start third
		this.parser.TryParse("[21:16:01] You hit Draekon for 93 (-17) damage!", out var second, out _, out _);
		second!.SpellName.Should().Be("Resonant Concussion", "second AE hit should also carry spell name");

		var third = this.parser.FlushPending();
	...
	public void TryParse_HitWithNoConfirmedSpell_OutsideWindow_SpellNameIsNull()
	{
		...
		this.parser.TryParse("[21:06:24] You hit Vareth for 39 (-7) damage!", out _, out _, out _);
	...
	public void TryParse_DotTicks_SubsequentTicksUseConfirmedSpellName()
	{
		...
		var result = this.parser.TryParse("[21:06:24] You hit Vareth for 39 (-7) damage!", out var flushed, out _, out _);
	...
	public void TryParse_SpellCastFlushesStaleDealtPending()
	{
		...
		var result = this.parser.TryParse("[21:06:20] You cast a Extinguish Lifeforce spell!", out var flushed, out _, out _);
	...
	// ── DoT / named-spell ticks ──────────────────────────────────────────────

	[Fact]
	public void TryParse_DotNamedHit_SpellNameTakenFromLine()
	{
		this.parser.TryParse("[14:12:42] Your Fungal Blight hits Tower Captain for 110 damage!", out _, out _, out _);

		var ev = this.parser.FlushPending();
		ev.Should().NotBeNull();
		ev!.SpellName.Should().Be("Fungal Blight");
		ev.BaseDamage.Should().Be(110);
		ev.CritDamage.Should().Be(0);
		ev.IsDealt.Should().BeTrue();
	...
	public void TryParse_DotNamedHitFollowedByCrit_EmitsMergedEvent()
	{
		...
		var result = this.parser.TryParse("[14:12:42] Your Fungal Blight critically hits Tower Captain for an additional 65 damage!", out var damage, out _, out _);

		result.Should().BeTrue();
		damage.Should().NotBeNull();
		damage!.SpellName.Should().Be("Fungal Blight");
		damage.BaseDamage.Should().Be(110);
		damage.CritDamage.Should().Be(65);
		damage.TotalDamage.Should().Be(175);
		damage.IsDealt.Should().BeTrue();
	}

	[Fact]
	public void TryParse_DotNamedHitWithAbsorbed_ParsesAbsorbed()
	{
		this.parser.TryParse("[14:12:42] Your Fungal Blight hits Tower Captain for 110 (-20) damage!", out _, out _, out _);

		var ev = this.parser.FlushPending();
		ev!.BaseDamage.Should().Be(110);
		ev.Absorbed.Should().Be(20);
	}
	...
	{
		this.parser.TryParse("[00:24:49] You fire a Critical Shot 9!", out _, out _, out _);
		this.parser.TryParse("[00:24:50] You hit Kingleo for 73 (-32) damage!", out _, out _, out _);
	...
	public void TryParse_BowFireWithPetCastBetweenFireAndHit_HitCarriesShotNameNotPetSpell()
	{
		...
		this.parser.TryParse("[00:31:30] You fire a Critical Shot 9!", out _, out _, out _);
		this.parser.TryParse("[00:31:31] You cast a Hunter's Companion spell!", out _, out _, out _);
		this.parser.TryParse("[00:31:31] You hit Hormei for 489 (-219) damage!", out _, out _, out _);
	...
	public void TryParse_TwoConsecutiveBowShots_EachHitCarriesItsOwnShotName()
	{
		...
		this.parser.TryParse("[00:24:49] You fire a Critical Shot 9!", out _, out _, out _);
		this.parser.TryParse("[00:24:50] You hit Kingleo for 73 (-32) damage!", out _, out _, out _);
		this.parser.TryParse("[00:24:51] You fire a Tempestuous Shot 9!", out var first, out _, out _);
		first!.SpellName.Should().Be("Critical Shot 9");

		// Second shot
		this.parser.TryParse("[00:24:52] You hit Kingleo for 255 (-85) damage!", out _, out _, out _);
		var second = this.parser.FlushPending();
	...
	public void TryParse_BowShotConfirmedThenHitOutsideWindow_UsesConfirmedBowShotName()
	{
		...
		this.parser.TryParse("[00:25:20] You hit Kingleo for 35 damage!", out var first, out _, out _);
		first!.SpellName.Should().Be("Critical Shot 9");
	...
	public void FlushPending_WithDealtPending_ReturnsEventAndClearsPending()
	{
		...
		this.parser.FlushPending().Should().BeNull("should be cleared after first flush");
	}

	[Fact]
	public void FlushPending_NoPending_ReturnsNull()
	{
		this.parser.FlushPending().Should().BeNull();
	}
	...
	{
		this.parser.TryParse("[21:55:49] You are healed by Pelgath for 800 hit points.", out _, out var heal, out _);
	...
	{
		var result = this.parser.TryParse("[21:55:49] Sylvorn is fully healed.", out _, out var heal, out _);
	...
	{
		this.parser.TryParse("[13:00:40] You perform your Clan's Call perfectly! (+90, Growth Rate: 1.21)", out _, out _, out _);
		this.parser.TryParse("[13:00:40] You attack Templar with your Fang of Everlasting Twilight and hit for 235 damage! (Damage Modifier: 1772)", out _, out _, out _);
	...
	public void TryParse_StylePerformed_MultiHit_AllHitsCarryStyleName()
	{
		...
		this.parser.TryParse("[13:00:40] You perform your Clan's Call perfectly! (+90, Growth Rate: 1.21)", out _, out _, out _);
		this.parser.TryParse("[13:00:40] You attack Templar with your Fang of Everlasting Twilight and hit for 235 damage! (Damage Modifier: 1772)", out _, out _, out _);
		this.parser.TryParse("[13:00:40] You attack Templar with your Astral Mephitic Fang and hit for 157 damage! (Damage Modifier: 2094)", out var first, out _, out _);
		first.Should().NotBeNull();
		first!.StyleName.Should().Be("Clan's Call", "first hit in multi-hit should carry style name");

		this.parser.TryParse("[13:00:40] You attack Templar with your Fang of Everlasting Twilight and hit for 151 damage! (Damage Modifier: 1850)", out var second, out _, out _);
	...
	{
		this.parser.TryParse("[13:00:38] You fail to execute your Clan's Call perfectly!", out _, out _, out _);
		this.parser.TryParse("[13:00:38] You attack Templar with your Fang of Everlasting Twilight and hit for 127 damage! (Damage Modifier: 1553)", out _, out _, out _);
	...
	public void TryParse_StylePerformed_ThenNextStyleOverridesPrevious()
	{
		...
		this.parser.TryParse("[13:00:38] You perform your Kelgor's Fist perfectly! (+50, Growth Rate: 1.10)", out _, out _, out _);
		this.parser.TryParse("[13:00:38] You attack Templar with your Fang of Everlasting Twilight and hit for 127 damage! (Damage Modifier: 1553)", out _, out _, out _);

		// Second style immediately after
		this.parser.TryParse("[13:00:40] You perform your Clan's Call perfectly! (+90, Growth Rate: 1.21)", out var first, out _, out _);
		first.Should().NotBeNull();
	...
	public void TryParse_StylePerformed_OutsideOneSecondWindow_StyleNameIsNull()
	{
		...
		this.parser.TryParse("[13:00:38] You perform your Clan's Call perfectly! (+90, Growth Rate: 1.21)", out _, out _, out _);
		this.parser.TryParse("[13:00:45] You attack Templar with your Fang of Everlasting Twilight and hit for 235 damage! (Damage Modifier: 1772)", out _, out _, out _);
	...
	public void TryParse_WeaponAttackWithMeleeCrit_CritDamageIncluded()
	{
		...
		this.parser.TryParse("[13:00:40] You attack Templar with your Astral Mephitic Fang and hit for 157 damage! (Damage Modifier: 2094)", out _, out _, out _);
		var result = this.parser.TryParse("You critically hit Templar for an additional 50 damage! (Crit Chance: 23%)", out var damage, out _, out _);
	...
	public void TryParse_StyledWeaponAttackWithMeleeCrit_BothStyleAndCritPreserved()
	{
		this.parser.TryParse("[13:00:40] You perform your Clan's Call perfectly! (+90, Growth Rate: 1.21)", out _, out _, out _);
		this.parser.TryParse("[13:00:40] You attack Templar with your Fang of Everlasting Twilight and hit for 235 damage! (Damage Modifier: 1772)", out _, out _, out _);
		var result = this.parser.TryParse("You critically hit Templar for an additional 80 damage! (Crit Chance: 23%)", out var damage, out _, out _);
	...
	public void TryParse_YouHitWithNoSpellAttribution_TreatedAsWeaponAttack()
	{
		...
		this.parser.TryParse("[13:00:40] You hit Templar for 150 damage!", out _, out _, out _);
	...
	public void TryParse_YouHitWithNoSpellAttribution_WithActivestyle_CarriesStyleName()
	{
		...
		this.parser.TryParse("[13:00:40] You perform your Kelgor's Fist perfectly! (+50, Growth Rate: 1.10)", out _, out _, out _);
		this.parser.TryParse("[13:00:40] You hit Templar for 150 damage!", out _, out _, out _);
	...
	public void TryParse_YouHitWithSpellAttribution_TreatedAsSpell()
	{
		...
		this.parser.TryParse("[13:00:38] You cast a Extinguish Lifeforce spell!", out _, out _, out _);
		this.parser.TryParse("[13:00:38] You hit Templar for 150 damage!", out _, out _, out _);
	...
	public void TryParse_StylePrepare_NpcHit_CarriesStyleName()
	{
		...
		this.parser.TryParse("[14:42:56] You prepare to perform a Totemic Fear!", out _, out _, out _);
		this.parser.TryParse("[14:42:57] You attack Guardian with your Fang of Everlasting Twilight and hit for 163 (-48) damage! (Damage Modifier: 1997)", out _, out _, out _);
	...
	public void TryParse_StylePrepare_ThenFail_NpcHit_NoStyleName()
	{
		...
		this.parser.TryParse("[14:42:56] You prepare to perform a Totemic Fear!", out _, out _, out _);
		this.parser.TryParse("[14:42:58] You fail to execute your Totemic Fear perfectly!", out _, out _, out _);
		this.parser.TryParse("[14:42:58] You attack Guardian with your Fang of Everlasting Twilight and hit for 149 (-44) damage! (Damage Modifier: 1822)", out var damage, out _, out _);

		var ev = damage ?? this.parser.FlushPending();
		ev.Should().NotBeNull();
	...
	public void TryParse_StylePrepare_AnPrefix_CarriesStyleName()
	{
		...
		this.parser.TryParse("[14:43:10] You prepare to perform an Uppercut!", out _, out _, out _);
		this.parser.TryParse("[14:43:11] You attack Mob with your Sword and hit for 100 damage! (Damage Modifier: 1000)", out _, out _, out _);
	...
	public void TryParse_StyleChainFallback_HitAttributedToOpeningStyle()
	{
		...
		this.parser.TryParse("[14:56:28] You must perform the Totemic Fear style before this one!", out _, out _, out _);
		this.parser.TryParse("[14:56:28] You attack Druid with your Fang of Everlasting Twilight and hit for 180 (-31) damage! (Damage Modifier: 2204)", out _, out _, out _);
	...
	public void TryParse_StyleChainFallback_MultiHit_AllSwingsAttributed()
	{
		...
		this.parser.TryParse("[14:56:30] You must perform the Totemic Fear style before this one!", out _, out _, out _);
		this.parser.TryParse("[14:56:30] You attack Druid with your Astral Mephitic Fang and hit for 174 (-30) damage! (Damage Modifier: 2311)", out _, out _, out _);
		this.parser.TryParse("[14:56:30] You attack Druid with your Fang of Everlasting Twilight and hit for 182 (-32) damage! (Damage Modifier: 2234)", out var hit2, out _, out _);

		hit2.Should().NotBeNull("second weapon attack flushes the first");
		hit2!.StyleName.Should().Be("Totemic Fear");

		var hit3 = this.parser.FlushPending();
