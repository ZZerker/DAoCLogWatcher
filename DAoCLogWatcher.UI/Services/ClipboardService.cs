...
public static class ClipboardService
{
	...
	private static void SetClipboardWin32(RenderTargetBitmap avBitmap)
	{
		...
		try
		{
			avBitmap.CopyPixels(new PixelRect(0, 0, w, h), gcHandle.AddrOfPinnedObject(), pixelsBgra.Length, stride);
		}
		finally
		{
			gcHandle.Free();
		...
		const int V5_SIZE = 124;
		const uint CF_DIBV5 = 17;

		var dib = new byte[V5_SIZE + pixelsBgra.Length];
		var s = dib.AsSpan();
		WriteLe32(s, 0, V5_SIZE); // bV5Size
		...
		pixelsBgra.CopyTo(dib.AsMemory(V5_SIZE));
