...
public partial class RealmPointSummary : ObservableObject
{
    ...
    [ObservableProperty]
    private int assaultOrder;

    [ObservableProperty]
    private int supportActivity;

    [ObservableProperty]
    private int relicCapture;

    [ObservableProperty]
    private int timedMissions;
    ...
    [ObservableProperty]
    private int assaultOrderRP;

    [ObservableProperty]
    private int supportActivityRP;

    [ObservableProperty]
    private int relicCaptureRP;

    [ObservableProperty]
    private int timedMissionsRP;
    ...
    public double RpsPerHour
    {
        get
        {
            ...
            if (this.LastEntryTime.HasValue)
            {
                var timeSinceLastEntry = now - this.LastEntryTime.Value;
                endTime = timeSinceLastEntry.TotalHours < 1.0 ? now : this.LastEntryTime.Value;
            }
            else
            {
                endTime = now;
